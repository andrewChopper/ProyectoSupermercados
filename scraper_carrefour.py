...
def ejecutar_carrefour():
    ...
    try:
        # Intentamos el scraping en vivo
        for nombre_cat, query_busqueda in CATEGORIAS_OBJETIVO.items():
            ...
            if resp.status_code == 200:
                ...
                # Si hay error 403 o similar, forzamos el salto al fallback
                raise Exception(f"Error de red: {resp.status_code}")

        # Si llegamos aquí y tenemos datos, los devolvemos
        if datos_totales:
            return pd.DataFrame(datos_totales)
        else:
            raise Exception("No se encontraron productos en la búsqueda")

    except Exception as e:
        # --- PLAN B: CARGAR CSV SI FALLA EL SCRAPING ---
        print(f"Scraping fallido ({e}). Cargando respaldo...")

        if os.path.exists(archivo_respaldo):
            df_respaldo = pd.read_csv(archivo_respaldo)
            # Añadimos la bandera para avisar en el FrontEnd
            df_respaldo['Es_Respaldo'] = True
            return df_respaldo
        else:
            # Si no hay ni scraping ni archivo, devolvemos tabla vacía con estructura
            cols = ['Categoría', 'Producto', 'Origen', 'Proveedor', 'Precio', 'Unidad']
